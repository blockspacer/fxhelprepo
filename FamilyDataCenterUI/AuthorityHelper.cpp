...
namespace
{
    const wchar_t* family_data_pre = L"FamilyData_";
...
bool AuthorityHelper::LoadFailyaDataAuthority(FamilyDataAuthority* authority)
{
    ...
    authority->username = rootdata.get("username", "").asString(); 
    ...
    const FamilyDataAuthority& authority, std::wstring* display)
{
    ...
    *display = std::wstring(L" 用户名: ") + base::UTF8ToWide(authority.username);
