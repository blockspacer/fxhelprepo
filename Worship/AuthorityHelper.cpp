...
bool AuthorityHelper::LoadUserTrackerAuthority(WorshipAuthority* authority)
{
...
bool AuthorityHelper::GetTrackerAuthorityDisplayInfo(
    const WorshipAuthority& authority, std::wstring* display)
{
    ...
    *display = std::wstring(L" ID: ") + userid;
    *display += L" 期限: " + expiredwstr;
    return true;
}